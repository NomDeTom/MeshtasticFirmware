...
for line in open(run / "events.jsonl", encoding="utf-8"):
    e = json.loads(line)
    if e.get("event") == "stimulus_rf_peer_done" or e.get("kind") == "stimulus_rf_peer_done":
...
for row, obs in sorted(seen.items()):
    n = min(sent.get(row, {}).values() or [0]) or 1 + max(i for o in obs.values() for _, i in o)
    stats = {"n": n}
    for w in ("witness", "lr"):
        got = obs.get(w, set())
        both = sum(1 for i in range(n) if ("dut", i) in got and ("peer", i) in got)
        one = sum(1 for i in range(n) if (("dut", i) in got) != (("peer", i) in got))
        stats[w] = {"both": both, "one": one, "none": n - both - one,
                    "dut": sum(1 for s, _ in got if s == "dut"), "peer": sum(1 for s, _ in got if s == "peer")}
    anyw = obs.get("witness", set()) | obs.get("lr", set())
    stats["any_both"] = sum(1 for i in range(n) if ("dut", i) in anyw and ("peer", i) in anyw)
...
for row, s in rows.items():
    n = s["n"]
    print(f"{row:10} n={n:3}  any-witness both={s['any_both']:3} ({s['any_both']/n:5.1%})  "
          f"cross-heard dut<-peer {s['dut_heard_peer']:3} peer<-dut {s['peer_heard_dut']:3}")
    for w in ("witness", "lr"):
        x = s[w]
        print(f"{'':12}{w:8} both {x['both']:3}  one {x['one']:3}  none {x['none']:3}   dut {x['dut']:3} peer {x['peer']:3}")
...
for short in sorted({r.split("-")[1] for r in rows}):
    ...
    if not (a and b):
        continue
    for label, key, denom in (("pairs both delivered", "any_both", 1), ("dut decoded peer", "dut_heard_peer", 1),
                              ("peer decoded dut", "peer_heard_dut", 1)):
        x1, n1, x2, n2 = a[key], a["n"], b[key], b["n"]
        p = fisher_two_sided(x1, n1 - x1, x2, n2 - x2)
        print(f"  {short} {label:22} dev {x1}/{n1} ({x1/n1:5.1%})  lbt {x2}/{n2} ({x2/n2:5.1%})  p={p:.4f}")
