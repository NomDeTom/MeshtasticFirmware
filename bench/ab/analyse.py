...
res = {}
print(
    f"{'key':3} {'label':12} {'snd':>3} {'B':>4} {'n':>3}  {'whole':>13}  {'x-heard':>13}  gap(airtimes) p10/med/p90  listeners"
)
for key, label in groups:
    ...
    n = len(idx)
    anyl = (
        set().union(*(seen[(key, label, w)] for w in listeners)) if listeners else set()
    )
    ...
    gaps.sort()
    g = (
        f"{gaps[len(gaps) // 10]:.1f}/{st.median(gaps):.1f}/{gaps[9 * len(gaps) // 10]:.1f}"
        if gaps
        else "-"
    )
    per = " ".join(
        f"{w}:{sum(1 for i in idx if all((s, i) in seen[(key, label, w)] for s in senders))}"
        for w in listeners
    )
    pc = lambda x, t: f"{x:4}/{t:<4}({x / t:4.0%})" if t else "-"  # noqa: E731
    print(
        f"{key:3} {label:12} {len(senders):3} {size[(key, label)]:4} {n:3}  {pc(whole, n)}  {pc(xh, xn)}  {g:>20}  {per}"
    )
...
for key, label in groups:
    ref = (
        "dev"
        if label == "lbt"
        else ("base" if label != "base" and (key, "base") in res else None)
    )
    ...
    print(
        f"  {key} {label:12} vs {ref:5}: whole {a['whole']}/{a['n']} -> {b['whole']}/{b['n']} p={pw:.3f}"
...
if "--links" in sys.argv:
    ...
    print(
        f"{'key':3} {'label':12} " + " ".join(f"{s[:4]}>{w[:4]:<4}" for s, w in links)
    )
    for key, label in groups:
        row = []
        for s, w in links:
