...
NF = re.compile(
    r"BENCH nf: n=(\d+) skip=(\d+)(?: p50=(-?\d+) p90=(-?\d+) min=(-?\d+) max=(-?\d+))?"
)
args = [a for a in sys.argv[1:] if not a.startswith("--")]
window = (
    int(sys.argv[sys.argv.index("--window") + 1]) if "--window" in sys.argv else 300
)
...
def summarise(keyfn, title):
    ...
    for k in sorted(agg):
        ...
        print(
            f"{' '.join(map(str, k)):22} {fl} {p9} {a['max']:6} {100 * a['skip'] / tot if tot else 0:6.1f} {len(a['p50']):5}"
        )
...
summarise(
    lambda ts, node: (
        time.strftime("%H:%M", time.localtime(ts // window * window)),
        node,
    ),
...
if variants:
    summarise(
        lambda ts, node: (label_at(ts), node) if label_at(ts) else None,
        "Per sweep variant",
    )

# -- passive probe lines (knobs8): 'BENCH probe: n= skip= cad_busy= rx_busy= rssi_busy= rssi_p50= floor='
PROBE = re.compile(
    r"BENCH probe: n=(\d+) skip=(\d+) cad_busy=(\d+) rx_busy=(\d+) rssi_busy=(\d+) rssi_p50=(-?\d+)"
)
...
if probe:
    ...
    for k in sorted(probe):
        ...
        if n:
            print(
                f"{' '.join(k):22} {n:5} {100 * c / n:5.0f}% {100 * r / n:6.0f}% {100 * e / n:7.0f}% {st.median(s):9.0f}"
            )
