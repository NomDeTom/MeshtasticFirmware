...
VARIANTS = json.loads(
    (HERE / (sys.argv[1] if len(sys.argv) > 1 else "sweep2.json")).read_text()
)
...
while True:
    ...
    vlines = [
        re.search(r"variant (\w+):", l).group(1)
        for l in lines
        if re.search(r"variant (\w+):", l)
    ]
    ...
    for lab in LABELS:
        ...
        extra = {
            k: v[k]
            for k in ("size", "skew", "burst", "senders", "interferers", "node_knobs")
        ...
        if m:
            ok = m["n"] > 0
            evidence = [
                ("knobs", f"'{desc}' {json.dumps(extra) if extra else ''}"),
            ...
            results[lab] = {
                ...
                "error": None,
                "outcomes": [
                    {"name": k, "verdict": "PASS", "evidence": e} for k, e in evidence
                ],
                "started_at": m["t0"],
                "ended_at": m["t1"],
                "images": {"all": "knobs2"},
                ...
                "outcomes": [
                    {
                        "name": "knobs",
                        "verdict": "PLANNED",
                        "evidence": f"'{desc}' {json.dumps(extra) if extra else ''}",
                    }
                ],
            }
            counts["PLANNED"] += 1
    ...
    if done_all:
        ...
        waiting = (
            "FAILED: " + next((l for l in reversed(lines) if "Error" in l), "")[:160]
        )
                    ...
                    json.dumps(
                        {
                            "ts": time.time(),
                            "node": (
                                mm.group(1) if mm and mm.group(1) in NODES else "runner"
                            ),
                            "source": "runner",
                            "line": l,
                        }
