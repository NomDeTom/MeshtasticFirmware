...
sent, seen, skew_of, frac, t_of, senders_of = defaultdict(set), defaultdict(set), {}, {}, {}, defaultdict(set)
for f in sys.argv[1:]:
    for line in open(f, encoding="utf-8"):
        d = json.loads(line)
        if d["kind"] == "sround":
            offs = sorted(d["offsets"].values())
            skew_of[(d["label"], d["i"])] = round(offs[1] - offs[0], 3) if len(offs) > 1 else 0
            continue
        if d["kind"] == "variant":
            sk = d.get("skews")
            if sk:
        ...
        if d["kind"] == "tx":
            sent[lab].add((s, i)); senders_of[lab].add(s)
        elif d["kind"] == "rx" and d["node"] != s:
            seen[lab].add((d["node"], s, i))


rounds = defaultdict(list)  # (block, strat, rot) -> [(whole, xh, xn, skewfrac, {sender: delivered})]
for lab in sent:
    ...
    for i in idx:
        if not all((s, i) in sent[lab] for s in ss):
            continue
        heard = {s: any((w, s, i) in seen[lab] for w in {n for n, _, _ in seen[lab]} - set(ss)) for s in ss}
        xh = sum((a, b, i) in seen[lab] for a, b in permutations(ss, 2))
        rounds[(block, strat, rot)].append((all(heard.values()), xh, len(ss) * (len(ss) - 1), frac.get((lab, i)), heard))
...
for block in blocks:
    ...
    print(f"{'strategy':10} {'whole':>15} {'x-heard':>15}  {'won/tied/lost vs ' + ref:>22}  {'MH OR':>6} {'p':>6}   per rotation whole%")
    for s in strats:
        ...
        for r in rots:
            ...
            wins += pb > pa; ties += pb == pa; losses += pb < pa
            strata.append((sum(x[0] for x in a), len(a), sum(x[0] for x in b), len(b)))
        orr, p = cmh(strata) if s != ref else (float("nan"), float("nan"))
        per = " ".join(f"{r}:{100 * sum(x[0] for x in rounds[(block, s, r)]) / len(rounds[(block, s, r)]):3.0f}"
                       for r in rots if rounds.get((block, s, r)))
        print(f"{s:10} {w:4}/{n:<4}({w / n:4.0%}) {xh:4}/{xn:<4}({xh / xn if xn else 0:4.0%})  "
              f"{'' if s == ref else f'{wins}/{ties}/{losses}':>22}  {orr:6.2f} {p:6.3f}   {per}")

    fr = sorted({x[3] for s in strats for r in rots for x in rounds.get((block, s, r), []) if x[3] is not None})
    if fr:
        ...
        for s in strats:
            cells = []
            for f in fr:
                rr = [x for r in rots for x in rounds.get((block, s, r), []) if x[3] == f]
                cells.append(f"{sum(x[0] for x in rr):3}/{len(rr):<3}{100 * sum(x[0] for x in rr) / len(rr):3.0f}%" if rr else "-")
            print(f"  {s:10} " + " ".join(f"{c:>10}" for c in cells))

    nodes = sorted({n for s in strats for r in rots for x in rounds.get((block, s, r), []) for n in x[4]})
    print(f"\n  per node as sender: share of its frames that reached a listener, block {block}")
    print(f"  {'strategy':10} " + " ".join(f"{n:>9}" for n in nodes))
    for s in strats:
        cells = []
        for nd in nodes:
            v = [x[4][nd] for r in rots for x in rounds.get((block, s, r), []) if nd in x[4]]
...
if anc:
    ...
    for t, r in anc:
        ...
        print(f"  {time.strftime('%H:%M', time.localtime(t))} {r:5} {w:3}/{len(rr):<3} {xh:3}/{xn:<3}")
