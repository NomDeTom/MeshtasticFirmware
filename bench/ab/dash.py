...
PLAN = [("dev", "flash", None)] + [("dev", ph, k) for k in ("SF", "NF", "NS") for ph in ("provision", "contend")]     + [("lbt", "flash", None)] + [("lbt", ph, k) for k in ("NS", "NF", "SF") for ph in ("provision", "contend")]
...
def measure(path):
    ...
    both = lambda s: sum(1 for i in pairs if ("dut", i) in s and ("peer", i) in s)  # noqa: E731
    return {
        "n": len(pairs), "both_any": both(anyw),
...
def result_for(rid, m):
    ...
    return {
        ...
        "outcomes": [{"name": k, "verdict": "PASS" if ok else "INVALID", "evidence": v} for k, v in ev],
        "images": {"all": "ab_develop" if rid.endswith("dev") else "ab_lbt"},
        "release_representative": True,
        "started_at": m["t0"], "ended_at": m["t1"],
...
while True:
    ...
    if cur != last_marker:
        ...
        with open(OUT / "events.jsonl", "a", encoding="utf-8") as f:
            f.write(json.dumps({"ts": time.time(), "kind": "lean_phase", "line": cur}) + "\n")

    # rows finished = contend markers followed by a later marker (or the end)
    contend_rows = [f"C-{m.group(2)}-{m.group(1)}" for l in markers
                    if (m := re.match(r"=== (\w+) (\w+): contend", l))]
    finished = contend_rows[:-1] if (cur and ": contend" in cur and not done_all) else contend_rows
    results = {}
    for rid in finished:
        f = row_file(rid)
        if f:
    ...
    if stage == "4-execute" and row and not done_all:
        ...
        if f and os.path.getmtime(f) >= marker_seen_at - 5:
            mm = measure(f)
            waiting = (f"{row}: {mm['n']}/{PAIRS[row.split('-')[1]]} pairs sent, "
    ...
    if done_all:
        ...
        waiting = f"FAILED: {next((l for l in reversed(lines) if 'Error' in l), '')[:160]}"
    ...
    for rid in ROWS:
        ...
        if rid == row and stage == "4-execute" and not done_all:
            f = row_file(rid)
            live = result_for(rid, measure(f)) if f and os.path.getmtime(f) >= marker_seen_at - 5 else None
            table[rid] = {**(live or {"outcomes": []}), "scenario_id": rid, "verdict": "RUNNING", "error": None}
        else:
    ...
    for n in NODES:
        ...
        ports[n] = {"role": NODE_ROLE[n], "firmware": fw, "declared_board": "NRF52_PROMICRO_DIY",
                    "state": ("leased" if busy and stage in ("2-flash", "3-provision") else
                              "held" if stage == "4-execute" else "idle"),
                    "capture": "protobuf api (per row)"}

    # schedule: one step per phase of the plan, marked from the markers seen so far
    seen = [(m.group(1), m.group(3), m.group(2)) for l in markers
            if (m := re.match(r"=== (\w+)(?: (\w+))?: (\w+)", l))]
    steps = []
    for idx, (lab, ph, key) in enumerate(PLAN):
        status = "planned"
        if (lab, ph, key) in seen:
            status = "running" if seen[-1] == (lab, ph, key) and not (done_all or failed) else "done"
            if failed and seen[-1] == (lab, ph, key):
                status = "failed"
        budget = BUDGET[ph][key] if ph == "contend" else BUDGET[ph]
        steps.append({"id": f"{lab}:{ph}:{key}", "name": f"{lab} {ph} {key or 'all nodes'}", "budget_s": budget,
                      "detail": "", "kind": ph, "node": None, "status": status, "recorded_status": status,
                      "over_by_s": None, "outcome": None, "elapsed_s": None, "overran": False, "children": []})
    ...
    if new:
        with open(OUT / "logs.jsonl", "a", encoding="utf-8") as f:
            for l in new:
                ...
                f.write(json.dumps({"ts": time.time(), "node": node, "source": "runner", "line": l}) + "\n")
        mirrored = len(lines)
    ...
    write("state.json", {
        ...
        "stage": stage, "row": row, "done": len(results), "total": len(ROWS),
        "waiting_for": waiting, "waiting_since": marker_seen_at, "counts": counts,
        "scenarios": ROWS,
        "nodes": [{"name": n, "serial_number": sn, "role": NODE_ROLE[n], "board": "NRF52_PROMICRO_DIY"}
                  for n, sn in NODES.items()],
        "ports": ports, "schedule": schedule,
    })
    if not (done_all or failed):
        with open(OUT / "status.jsonl", "a", encoding="utf-8") as f:
            f.write(json.dumps({"ts": time.time(), "component": "runner", "stage": stage, "row": row,
                                "waiting_for": waiting, "done": len(results), "total": len(ROWS)}) + "\n")
    else:
        break
    time.sleep(5)
