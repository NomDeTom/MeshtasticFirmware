...
inline uint32_t getSafeMillis()
{
    return skipZero(getMillis());
}

/// Arm a deadline delayMs from now, never 0. The sum is what has to dodge 0 - a non-zero read plus a
/// delay lands there once per wrap - so this is not getSafeMillis() + delayMs.
inline uint32_t deadlineIn(uint32_t delayMs)
{
