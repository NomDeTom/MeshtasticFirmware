...
void benchKnobsLog()
{
    ...
    LOG_INFO("BENCH knobs: lbt=%s sym=%u cwmin=%d cwmax=%d slot=%d fixed=%u nobackoff=%d pre=%s sync=%d iq=%d pwr=%d "
             "detpeak=%d detmin=%d agc=%ld li=%d nf=%u rssim=%d floor=%d dc=%u/%u",
             LBT_NAMES[k.lbt], k.cadSymbols, k.cwMin, k.cwMax, k.slotMs, k.fixedMs, k.noBackoff ? 1 : 0, PRE_NAMES[k.pre],
             k.syncWord, k.iqInvert, k.txPower, k.detPeak, k.detMin, (long)k.agcMs, k.li, k.nfMs, k.rssiMargin, k.floorDbm, k.dcSleepMs, k.dcWakeMs);
}
