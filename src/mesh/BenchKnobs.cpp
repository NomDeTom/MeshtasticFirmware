...
bool benchKnobsHandleCommand(const char *text, size_t len)
{
    ...
    for (char *tok = strtok_r(nullptr, " ", &save); tok; tok = strtok_r(nullptr, " ", &save)) {
        ...
        *eq = 0;
        ...
        } else if (strcmp(key, "rxcont") == 0) {
            next.rxCont = num != 0;
            radioChanged = true;
        } else if (strcmp(key, "trig") == 0) {
            next.trigNode = (uint32_t)strtoul(val, nullptr, 0);
    ...
    // The RSSI modes imply continuous RX, so switching in or out of them restarts RX in the other mode.
    radioChanged |= next.usesRssi() != benchKnobs.usesRssi();
    // RadioLib keeps IQ and re-sends it with every packet-param write, so "don't touch" after iq=1 would
    // leave the chip inverted: apply standard IQ once, then return to "don't touch".
    const bool restoreIq = benchKnobs.iqInvert == 1 && next.iqInvert == -1;
    if (restoreIq) {
        next.iqInvert = 0;
        radioChanged = true;
    }
