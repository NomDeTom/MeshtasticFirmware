...
static const Module::RfSwitchMode_t rfswitch_table[] = {
    {LR2021::MODE_STBY, {}}, {LR2021::MODE_RX, {}}, {LR2021::MODE_TX, {}}, {LR2021::MODE_TX_HF, {}}, END_OF_MODE_TABLE,
};
#endif

// Particular boards might define a different max power based on what their hardware can do, default to max power output if not
// specified (may be dangerous if using external PA and LR2021 power config forgotten)
#if ARCH_PORTDUINO
...
#endif
#ifndef LR2021_MAX_POWER
#define LR2021_MAX_POWER 13
#endif

template <typename T>
LR2021Interface<T>::LR2021Interface(LockingArduinoHal *hal, RADIOLIB_PIN_TYPE cs, RADIOLIB_PIN_TYPE irq, RADIOLIB_PIN_TYPE rst,
                                    RADIOLIB_PIN_TYPE busy)
    : RadioLibInterface(hal, cs, irq, rst, busy, &lora), lora(&module)
{
    LOG_WARN("LR2021Interface(cs=%d, irq=%d, rst=%d, busy=%d)", cs, irq, rst, busy);
}

/// Initialise the Driver transport hardware and software.
/// Make sure the Driver is properly configured before calling init().
/// \return true if initialisation succeeded.
template <typename T> bool LR2021Interface<T>::init()
{
...
// FIXME: correct logic to default to not using TCXO if no voltage is specified for LR2021_DIO3_TCXO_VOLTAGE
    ...
    if (res == RADIOLIB_ERR_SPI_CMD_FAILED) {
        LOG_WARN("LR2021 init failed with %d (SPI_CMD_FAILED), retrying after delay...", res);
        delay(100);
        res = lora.begin(getFreq(), bw, sf, cr, syncWord, power, preambleLength, tcxoVoltage);
    }

    // \todo Display actual typename of the adapter, not just `LR2021`
    LOG_INFO("LR2021 init result %d", res);
    if (res == RADIOLIB_ERR_CHIP_NOT_FOUND || res == RADIOLIB_ERR_SPI_CMD_FAILED)
        return false;

    // LR2021VersionInfo_t version;
    // res = lora.getVersionInfo(&version);
    // if (res == RADIOLIB_ERR_NONE)
    //     LOG_DEBUG("LR2021 Device %d, HW %d, FW %d.%d, WiFi %d.%d, GNSS %d.%d", version.device, version.hardware,
    //     version.fwMajor,
...
template <typename T> bool LR2021Interface<T>::reconfigure()
{
    ...
    if (power > LR2021_MAX_POWER) // This chip has lower power limits than some
        power = LR2021_MAX_POWER;
    if ((power > LR2021_MAX_POWER) && (config.lora.region == meshtastic_Config_LoRaConfig_RegionCode_LORA_24)) // 2.4G power limit
        power = LR2021_MAX_POWER;
...
template <typename T> void LR2021Interface<T>::disableInterrupt()
{
    lora.clearIrqAction();
}

template <typename T> void LR2021Interface<T>::setStandby()
{
    ...
    if (err != RADIOLIB_ERR_NONE) {
        LOG_DEBUG("LR2021 standby failed with error %d", err);
    }
 ...
 */
template <typename T> void LR2021Interface<T>::addReceiveMetadata(meshtastic_MeshPacket *mp)
{
 ...
 */
template <typename T> void LR2021Interface<T>::configHardwareForSend()
{
...
template <typename T> void LR2021Interface<T>::startReceive()
{
#ifdef SLEEP_ONLY
    sleep();
#else
...
template <typename T> bool LR2021Interface<T>::isChannelActive()
{
...
template <typename T> bool LR2021Interface<T>::isActivelyReceiving()
{
    // The IRQ status will be cleared when we start our read operation. Check if we've started a header, but haven't yet
    // received and handled the interrupt for reading the packet/handling errors.
    return receiveDetected(lora.getIrqStatus(), RADIOLIB_LR2021_IRQ_SYNCWORD_VALID, RADIOLIB_LR2021_IRQ_PREAMBLE_DETECTED);
}

#ifdef LR2021_AGC_RESET
template <typename T> void LR2021Interface<T>::resetAGC()
{
    // Safety: don't reset mid-packet
    if (sendingPacket != NULL || (isReceiving && isActivelyReceiving()))
        return;

    LOG_DEBUG("LR2021 AGC reset: warm sleep + Calibrate(0x3F)");

    // 1. Warm sleep — powers down the analog frontend, resetting AGC state
    lora.sleep(true, 0);

    // 2. Wake to RC standby for stable calibration
    lora.standby(RADIOLIB_LR2021_STANDBY_RC, true);

    // 3. Calibrate all blocks (PLL, ADC, image, RC oscillators)
    //    calibrate() is protected on LR2021, so use raw SPI (same as internal implementation)
    uint8_t calData = RADIOLIB_LR2021_CALIBRATE_ALL;
    module.SPIwriteStream(RADIOLIB_LR2021_CMD_CALIBRATE, &calData, 1, true, true);
...
template <typename T> bool LR2021Interface<T>::sleep()
{
    // \todo Display actual typename of the adapter, not just `LR2021`
    LOG_DEBUG("LR2021 entering sleep mode");
