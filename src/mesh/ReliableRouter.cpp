...
void ReliableRouter::sniffReceived(const meshtastic_MeshPacket *p, const meshtastic_Routing *c)
{
    if (isToUs(p)) { // ignore ack/nak/want_ack packets that are not address to us (we only handle 0 hop reliability)
        ...
        if (p->which_payload_variant == meshtastic_MeshPacket_decoded_tag && c &&
            ...
            // Only for a DM of ours still pending to the node that says it lacks our key. The NAK is a
            // cleartext-addressed frame anyone holding the channel can forge; without this, one forged
            // NAK makes any node broadcast a NodeInfo to any address.
            const PendingPacket *ours = findPendingPacket(getNodeNum(), p->decoded.request_id);
            if (ours && ours->packet->to == getFrom(p) && owner.public_key.size == 32) {
