...
```bash
cd sim
python3 -m pytest sfpp/test_mesh.py -q          # 98 tests. This is a gate, not a formality.

# one scenario, with statistics and charts written beside the JSON
python3 -m sfpp.campaign --hours 24 --seed 1 --protocol sr --out /tmp/r/run.json

# a swept arm, three seeds
...
./run-blocks.sh /tmp/r 440001 R-oversubscribed R-congestion-input R-repeats
...
`sweep.py` prints the statistics report and renders a chart automatically, so a block's output is
complete without a second command.
...
| `--platform-mix`         | `uniform`        | board mix; decides each node's hot-store size                                                                         |
...
| `--profile`                 | `2.8`       | rule set: `2.8` (this tree), `2.5-approx`, `pre-fold-in` (`legacy` accepted as an alias)   |
| `--profile-flag NAME=VALUE` | -           | override one rule, repeatable. Pre-2.5 pathologies live here rather than as a fake version |
| `--rebroadcast-mode`        | `ALL`       | `ALL`, `ALL_SKIP_DECODING`, `LOCAL_ONLY`, `KNOWN_ONLY`, `CORE_PORTNUMS_ONLY`, `NONE`       |
| `--legacy-fraction`         | 0.0         | share of nodes on the older profile, drawn at random not by degree                         |
...
**On `--profile`.** `2.8` is read from this tree line by line. `2.5-approx` is a **floor, not a
reconstruction** - 2.8 with the behaviours certainly newer than 2.5 turned off (next-hop routing, late
window, utilisation backoff, role-aware cancellation, hop upgrade) and everything else left at 2.8.
Intermediate versions are deliberately not offered because the diffs were never done, and inventing
2.6 and 2.7 boundaries would be worse than omitting them. `pre-fold-in` is **not a firmware version**:
it is this simulator's own earlier transport, retained so a result can be attributed to a rule change
rather than to the rewrite around it, and because the campaign's pre-fix results were measured on it.
...
thirds of rebroadcast attempts in rounds one to three.
...
| `--advert-interval-s` | 300         | interval-trigger period                                                                                                |
...
| `--catch-up-hours`    | -           | defer reconciliation to quiet hours, e.g. `02-06`                                                                      |
...
| `--out`       | -       | JSON path; charts land in `figures/` beside it     |
| `--no-charts` | off     | skip chart rendering                               |
...
| `clustered` | _k_ towns, sparse between      | 20.7       | 6        | what most regional meshes look like; between-town links are the bottleneck        |
| `corridor`  | long and thin, aspect 6:1      | 8.3        | 13       | a valley or coast road; hop limit binds hard, placement is nearly one-dimensional |
| `hub`       | dense core plus radial spokes  | 18.4       | 5        | the core hears everything, the spoke ends almost nothing                          |
| `chain`     | towns strung in a line         | 9.6 @16 km | **11**   | **the way to build a mesh wider than any hop limit that stays connected**         |
...
| ---------------- | ----------- | ----- | ------ | -------- | -------- | -------------- |
| `SHORT_FAST`     | −121.5      | 98    | 3.3    | 17       | **1**    | 111 ms         |
| `MEDIUM_FAST`    | −126.5      | 160   | 5.3    | 15       | 0        | 353 ms         |
| `LONG_FAST`      | −131.5      | 253   | 8.4    | 7        | 0        | 1190 ms        |
| `LONG_MODERATE`  | −134.5      | 332   | 11.1   | 6        | 0        | 3609 ms        |
| `LONG_SLOW`      | −137.0      | 410   | 13.7   | 5        | 0        | 6431 ms        |
| `VERY_LONG_SLOW` | −140.0      | 529   | 17.6   | 4        | 0        | 11289 ms       |

Preset feeds four paths: **sensitivity → the link graph** (21.5 dB across the range, so 98 links
against 529 on identical geometry - the dominant effect), airtime → contention → collisions, coding
